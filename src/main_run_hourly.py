...
from core.FloodModel import GridXAJHourlyModel
...
config = SimulationConfig(
    start_date = pd.to_datetime("2011/6/3 12:00"),
    end_date   = pd.to_datetime("2011/6/26 14:00"),
...
Restart = xr.open_dataset(r"E:\Learning\GXAJ_fromVB\test\Restart\RESTART_2011-06-03.nc")
model = GridXAJHourlyModel(config, geostatic, lumparams, calorder)
Restart.close()

forcing = xr.open_dataset("../test/GXAJ_Hourly_Forcing_2011_BJT.nc")
forcing = forcing.sel(time=slice(config.start_date, config.end_date))
...
Qobs = pd.read_csv("../test/Qobs/Tunxi_1H_20110604.csv")
plot_line(Qobs, title="Simulated Streamflow")
