...
class GridXAJHourlyModel:
    ...
    def GXAJ_drv(self, time, precip, evap, iStep):
        ...
        print(f"Running time step {iStep + 1}/{len(self.timeseries)}: {self.timeseries[iStep].strftime('%Y-%m-%d_%H:%M')}")
        ...
        if self.config.save_restart:
            self._save_Restart(f"{self.config.outdir}/RESTART_{self.timeseries[iStep].strftime('%Y-%m-%d_%H:%M')}.nc")
    ...
    def run(self, precip_series, evap_series, Restart: Optional[xr.Dataset] = None):
        ...
        self.timeseries = pd.date_range(self.config.start_date, self.config.end_date, freq='1h')
        ...
        for iStep in range(len(self.timeseries)):
            ...
            plot_2d(self.state.RunoffState.R, 
                    title=f"Runoff at {self.timeseries[iStep].strftime('%Y-%m-%d %H:%M')}", 
                    save_path=f"{self.config.outdir}/Runoff_{self.timeseries[iStep].strftime('%Y-%m-%d_%H:%M')}.png")
